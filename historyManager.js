...
window.PortfolioApp.HistoryManager = (() => {
  ...
  function register(id, closeFn) {
    ...
    /* ── Guard: registro duplicado ──────────────────────────
       Si el mismo id ya está en el stack (ej. doble-tap en
       móvil sobre el trigger antes de que el panel termine de
       abrir), NO empujamos una segunda entrada de historial.
       Eso dejaría _stack y history desincronizados y obligaría
       al usuario a presionar Back dos veces para salir.
       En su lugar: actualizamos closeFn (por si cambió) y
       salimos sin tocar el historial. */
    const existingIdx = _stackIndexOf(id);
    if (existingIdx !== -1) {
      console.warn('[HistoryManager] register() duplicado para "' + id + '" — ignorando pushState repetido.');
      _stack[existingIdx].closeFn = closeFn;
      return;
    }
  ...
  window.addEventListener('popstate', () => {
    ...
    /* NOTE (Engineering Pass v3.2, Milestone 1): a state-validation
       cross-check against e.state was drafted here but reverted.
       popstate's e.state reflects the entry being navigated TO
       (the previous panel, or null) — not the entry being closed —
       so a naive id comparison against `top.id` would misfire on
       the normal path, not just on desync. A correct version needs
       to compare e.state against _stack[_stack.length - 2] (the
       *next* entry down) instead. Left unimplemented pending review,
       to avoid risking the core Back-closes-panel behavior. */
