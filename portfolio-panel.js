/* ══════════════════════════════════════════════════════════
   portafolioPanel.js — Módulo Galería / Portafolio
   Sebastián Castillo Portfolio — v2.0
   ══════════════════════════════════════════════════════════ */
...
(function initPortafolioPanel() {
 ...
 /* ────────────────────────────────────────────
     DATA (Migrado a Repositorio Local)
  ──────────────────────────────────────────── */
const GALLERIES = [
  {
    id: 'g1', 
  ...
  /* ────────────────────────────────────────────
     BUILD PANEL
     Nota: #portRooms eliminado del panel.
     Las rooms se montan directamente en body.
  ──────────────────────────────────────────── */
  function buildPanel() {
  ...
  /* ────────────────────────────────────────────
     BUILD LIGHTBOX (en body, fuera del panel)
  ──────────────────────────────────────────── */
    ...
    GALLERIES.forEach((gallery, idx) => {
      ...
      const thumbsHtml = gallery.images.map((src, i) => `
        <div class="port-room-thumb" data-index="${i}" data-gallery="${idx}" role="button" tabindex="0" aria-label="Ver imagen ${i + 1}">
          <img src="${src}" alt="Fotografía ${i + 1} de ${gallery.title}" loading="lazy"/>
        </div>
      `).join('');
      ...
      /* Mount room directly on body — completely outside .port-panel */
      document.body.appendChild(room);
    });
  }

  /* ────────────────────────────────────────────
     ZOOM-PAN (grid de selección)
  ──────────────────────────────────────────── */
  function initZoomPan(panel) {
    panel.addEventListener('pointermove', e => {
      ...
      item.style.setProperty('--zoom-ox', ox);
      item.style.setProperty('--zoom-oy', oy);
    });
  }
  ...
  function closePanel() {
    ...
    /* Cerrar todas las salas (están en body, no en el panel) */
    document.querySelectorAll('.port-gallery-room.open').forEach(r => {
      r.classList.remove('open');
      r.setAttribute('aria-hidden', 'true');
    });
  ...
  function wireEvents(panel) {
    ...
    document.addEventListener('click', e => {
      ...
      if (backBtn) {
        e.stopPropagation();
        closeRoom(backBtn.dataset.room);
      }
    });

    /* Room thumbs → lightbox (rooms están en body) */
    document.addEventListener('click', e => {
      const thumb = e.target.closest('.port-room-thumb');
      if (thumb) {
        openLightbox(parseInt(thumb.dataset.gallery, 10), parseInt(thumb.dataset.index, 10));
      }
    });
    document.addEventListener('keydown', e => {
  ...
  function wireGaleriasButton() {
    /* Soporte para ambas variantes: id en HTML o último .module-btn */
  ...
  function initCarousel(ring) {
    ...
    CAROUSEL_IMGS.forEach((src, i) => {
      ...
      img.src = src; img.alt = GALLERIES[i].title; img.loading = 'lazy';
  ...
  function initCursor(panel) {
    ...
    /* Trackear movimiento tanto en el panel como en las salas */
    ...
    function onOver(e) {
      if (e.target.closest(INTERACTIVE)) { dot.classList.add('hovering'); ring.classList.add('hovering'); }
    }
    function onOut(e) {
      if (e.target.closest(INTERACTIVE)) { dot.classList.remove('hovering'); ring.classList.remove('hovering'); }
    }
    ...
    document.addEventListener('pointermove', e => {
      const inRoom = e.target.closest('.port-gallery-room.open');
      if (inRoom) onMove(e);
    });
    document.addEventListener('pointerover', e => {
      if (e.target.closest('.port-gallery-room.open')) onOver(e);
    });
    document.addEventListener('pointerout', e => {
      if (e.target.closest('.port-gallery-room.open')) onOut(e);
    });
  }
